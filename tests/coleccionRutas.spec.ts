...
describe('ColeccionRutas', () => {
  ...
  const ruta1 = new Ruta(1, 'ruta1', [1, 1], [2, 2], 20, 1, [1, 2, 3, 4], 4, 1);
  const ruta2 = new Ruta(2, 'ruta2', [1, 1], [2, 2], 30, 1, [1, 2], 5, 1);
  ...
  it('Se puede obtener una ruta de la colección', () => {
    coleccionRutas.addRuta(ruta2);
    expect(coleccionRutas.getRuta(1)).to.eql(ruta2);
  });
  it('Se puede obtener todas las rutas de la colección', () => {
    expect(coleccionRutas.getRutas()).to.have.lengthOf(2);
  });
  ...
  it('Se puede obtener una ruta dado el nombre', () => {
    expect(coleccionRutas.getRutaPorNombre('ruta1')).to.equal(ruta1);
  });
  it('Se puede obtener una ruta dado el indice', () => {
    expect(coleccionRutas.getItem(0)).to.equal(ruta1);
  });
